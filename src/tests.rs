...
fn trust_account_cant_trust_self() {
	new_test_ext().execute_with(|| {
		assert_noop!(TemplateModule::trust_account(RuntimeOrigin::signed(1), 1), Error::<Test>::TrustSelf);
	});
}
...
fn trust_account_already_trusted() {
	new_test_ext().execute_with(|| {
		assert_ok!(TemplateModule::trust_account(RuntimeOrigin::signed(1), 2));
		assert_noop!(TemplateModule::trust_account(RuntimeOrigin::signed(1), 2), Error::<Test>::AlreadyTrusted);
	});
}
...
fn untrust_account_not_trusted() {
	new_test_ext().execute_with(|| {
		assert_noop!(TemplateModule::untrust_account(RuntimeOrigin::signed(1), 2), Error::<Test>::NotTrusted);
	});
}
